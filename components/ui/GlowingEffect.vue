...
// Simple animation function since motion-v might not be available
function animate(from: number, to: number, options: { duration: number; ease: number[]; onUpdate: (value: number) => void }) {
...
function handleMove(e?: MouseEvent | PointerEvent | { x: number; y: number }) {
  ...
  animationFrame.value = requestAnimationFrame(() => {
    ...
    let targetAngle = (180 * Math.atan2(mouseY - (center[1] ?? 0), mouseX - (center[0] ?? 0))) / Math.PI + 90
